...
# Datasets for Downstream Document Control for us instance
...
# Create DAG runs every saturday at 5:00 AM UTC equivalent to 
with DAG(
    ...
    @task_group(group_id="run_us_aconex_users_glue_jobs")
    def run_us_users_glue_jobs(ProjectId):
        """Task Group to run all Glue jobs in parallel for a given project ID"""
        glue_job_tasks = []
        
        # User Project Task
        user_project_task = GlueJobOperator(
                task_id=f"aconex_us_user_project",
        ...
        user_projectrole_task = GlueJobOperator(
                task_id=f"aconex_us_user_projectrole",
    ...
    UserDirectory_job = GlueJobOperator(
        task_id="aconex_us_userDirectory",
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"aconex_us_users_raw_crawler",
        config={
    ...
    aconex_project_list_task >> run_us_users_glue_jobs.expand(ProjectId=aconex_project_list_task) >> raw_crawler >> raw_crawler_sensor
