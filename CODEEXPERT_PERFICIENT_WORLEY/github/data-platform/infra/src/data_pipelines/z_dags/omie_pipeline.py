...
dag_omie_config = Variable.get("omie_env", deserialize_json=True)
dag_config = Variable.get("env", deserialize_json=True)
omie_environment = dag_omie_config["environment"]
environment = dag_config["environment"]
region = "ap-southeast-2"
source_system_id = "omie"
domain_name = "engineering"
table_names = ["req","wfstatus","combo"]
...
glue_job_names = {
    ...
    "omie_raw_curated": f"worley-datalake-sydney-{environment}-glue-job-raw-to-curated-generic",
    "schema_change_detection": f"worley-datalake-sydney-{environment}-glue-job-schema-change-detection-generic"
}

payload = json.dumps({
          "dag_name": f"omie_{environment}_data_pipeline"
          })
...
# Create DAG
with DAG(
    dag_id=f"omie_{environment}_data_pipeline",
    schedule="0 9 * * 1-5",
    tags=["omie"],
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"omie_raw_crawler",
        config={
            "Name": f"worley-datalake-sydney-{environment}-glue-crawler-engineering-omie-raw",
        },
        poll_interval=5,
    ...
    def raw_to_curated():
        for table in table_names:
            raw_curated = GlueJobOperator(
                task_id=f"cur_{source_system_id}_{table}",
                job_name=glue_job_names["omie_raw_curated"],
                region_name=region,
                verbose=True,
                wait_for_completion=True,
                script_args={
                    "--source_system_id": f"database_{source_system_id}_curated",
                    "--metadata_type": f"curated#omie#{table}#job#iceberg",
                    "--start_date": execution_date,
                    "--end_date": execution_date,
                },
            )
