...
# Retrieve variables from airflow env variables to prod
...
job_data = [metadata[environment]['Project_VG']['job_data']['project'], metadata[environment]['Project_DAC']['job_data']['project']]
...
datasets = ["project_list", "TimePhased", "deliverable_gate","change_deliverable", "controlAccount_Snapshot"]
...
entity_sourcing = ["TimePhased", "controlAccount_Snapshot","deliverable_gate","change_deliverable"]
glue_job_names = {
...
payload = json.dumps({
          "dag_name": f"ecosys_xml_{environment}_data_pipeline"
          })
...
# Create DAG
with DAG(
    dag_id=f"ecosys_xml_{environment}_data_pipeline",
    schedule="0 5 * * *",
    tags=["ecosys"],
    ...
    def ecosys_intial_sourcing(secorg_id: str,source_name: str = "ecosys" ):
        for entity in initial_sourcing:
            src_raw_initial = GlueJobOperator(
              task_id=f"{source_name}_{entity}_initial_sourcing",
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"ecosys_project_raw_crawler",
        config={
    ...
    raw_crawler_sensor = GlueCrawlerSensor(
        task_id="wait_for_ecosys_project_raw_crawler",
    ...
    (
        ecosys_intial_sourcing.expand(secorg_id=secorg_ids)
