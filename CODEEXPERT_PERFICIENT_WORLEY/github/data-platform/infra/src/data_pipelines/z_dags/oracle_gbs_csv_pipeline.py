...
datasets = ["currency_exchange_rate", "currency_type", "adc_coa", "business_line", "capability_subsector_type", "coa_cost_type", "consulting_service_line", "customer_agreement", "expenditure_type", "external_organization", "global_job_code", "group_code", "hr_discipline", "hr_person_assignment", "internal_organization", "level_code", "location", "person", "person_bad", "person_type", "person_type_asgn", "project", "project_billing_type", "project_funding", "project_phase_types", "project_size_classification", "project_status_type", "project_type", "scope_of_service_type", "sector_service_unit", "unit_of_measure", "tasks", "project_phase", "customer", "data_ic_cost", "recon_br"]
...
glue_job_names = {
    ...
    "oraclegbs_raw_curated_upgraded": f"worley-datalake-sydney-{environment}-glue-job-raw-to-curated-generic-upgraded"
}
...
# Create DAG
with DAG(
    dag_id=f"oracle_gbs_csv_{environment}_data_pipeline",
    schedule="30 5 * * 1-5",
    tags=["oracle_gbs_csv"],
    ...
    sftp_and_unzipping = GlueJobOperator(
        ...
        script_args={
            ...
            "--bucket_key": 'finance/oracle_gbs/raw/'+'{{ ti.xcom_pull(task_ids="partition_path") }}',
    ...
    def oracle_gbs_csv_sourcing():
        ...
        csv_task = GlueJobOperator(
            ...
            script_args={
                ...
                "--connector_file_path" : 'finance/oracle_gbs/raw/'+'{{ ti.xcom_pull(task_ids="partition_path") }}'
            },
        )
    ...
    @task_group(group_id=f"curation_high_volume_datasets")
    def raw_to_curated_upgraded():
        for table in high_volume_datasets:
            raw_curated = GlueJobOperator(
                task_id=f"cur_{source_system_id}_{table}",
                job_name=glue_job_names["oraclegbs_raw_curated_upgraded"],
                region_name=region,
                verbose=True,
                wait_for_completion=True,
                script_args={
    ...
    s3_partition_path >> sftp_and_unzipping >> sourcing_tasks >> raw_crawler >> detect_schema_change() >> raw_to_curated() >> raw_to_curated_upgraded() >> run_finance_models_dag >> sns_notification_for_failure
