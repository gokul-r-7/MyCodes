...
spark = SparkSession.builder.appName("Update IDs").getOrCreate()
...
for mapped_value, original_key in reverse_mapping.items():
    selected_columns_df = selected_columns_df.withColumn(
        "Metric",
        F.when(F.col("Metric") == mapped_value, original_key).otherwise(F.col("Metric"))
    )
...
max_date_row = df_filled.agg(F.max('Date').alias('max_date')).collect()
max_date = max_date_row[0]['max_date']
yesterday_date = max_date
yesterday_values_df = df_filled.filter(F.col('Date') == yesterday_date)
yesterday_values_df = yesterday_values_df.withColumnRenamed('Value', 'Value_Yesterday')
df_filled_with_yesterday = df_filled.join(
    yesterday_values_df.select('Metric', 'Operating System Type', 'Value_Yesterday'),
    on=['Metric', 'Operating System Type'],
    how='left'
)
df_sorted = df_filled_with_yesterday.sort(['Date','Operating System Type'])
final_columns = [
    'id', 'Metric', 'Operating System Type', 
    'Value', 'Date', 'create_dt', 'Value_Yesterday'
]
df_final = df_sorted.select(final_columns)
df_final = df_final.orderBy(['id', 'Metric', 'Operating System Type'])
mask = df_final['id'] == 0

unique_combinations = df_final.filter(mask).dropDuplicates(['Metric', 'Operating System Type'])

unique_combinations_with_ids = unique_combinations.withColumn("new_id", F.row_number().over(Window.orderBy(F.monotonically_increasing_id())))

df_final = df_final.join(
    unique_combinations_with_ids.select('Metric', 'Operating System Type', 'new_id'),
    on=['Metric', 'Operating System Type'],
    how='left'
).withColumn('id', F.when(df_final['id'] == 0, F.col('new_id')).otherwise(df_final['id'])).drop('new_id')
output_path = 's3://cci-dig-aicoe-data-sb/processed/da/HealthScore/HC_Source_Data_Calc/'
