...
dag_pdm_config = Variable.get("pdm_env", deserialize_json=True)
dag_config = Variable.get("env", deserialize_json=True)
pdm_environment = dag_pdm_config["environment"]
environment = dag_config["environment"]
region = "ap-southeast-2"
source_system_id = "pdm"
domain_name = "engineering"
table_names = ["PDM_PIPE_DB"]
...
glue_job_names = {
    ...
    "pdm_raw_curated": f"worley-datalake-sydney-{environment}-glue-job-raw-to-curated-generic",
    "schema_change_detection": f"worley-datalake-sydney-{environment}-glue-job-schema-change-detection-generic"
}

payload = json.dumps({
          "dag_name": f"pdm_{environment}_data_pipeline"
          })
...
# Create DAG
with DAG(
    dag_id=f"pdm_{environment}_data_pipeline",
    schedule="15 7 * * 1-5",
    tags=["pdm"],
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"pdm_raw_crawler",
        config={
            "Name": f"worley-datalake-sydney-{environment}-glue-crawler-engineering-pdm-raw",
        },
        poll_interval=5,
    ...
    def raw_to_curated():
        for table in table_names:
            raw_curated = GlueJobOperator(
                task_id=f"cur_{source_system_id}_{table}",
                job_name=glue_job_names["pdm_raw_curated"],
                region_name=region,
                verbose=True,
                wait_for_completion=True,
                script_args={
                    "--source_system_id": f"database_{source_system_id}_curated",
                    "--metadata_type": f"curated#pdm#{table}#job#iceberg",
                    "--start_date": execution_date,
                    "--end_date": execution_date,
                },
            )
