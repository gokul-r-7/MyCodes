...
payload = json.dumps({
          "dag_name": "dbt_project_control_v2_p6_dag",
...
with DAG(
    dag_id="dbt_project_control_v2_p6_dag",
    ...
    dbt_task = DbtTaskGroup(
        group_id="project_control_p6_dbt_task",
        execution_config=execution_config,
        profile_config=profile_config,
        project_config=project_config,
        operator_args={
        ...
        render_config= RenderConfig(
            exclude=["tag:audit","tag:snowflake_models","tag:ecosys", "tag:v1"],
            select=["tag:p6","tag:v2"],
            load_method=LoadMode.DBT_LS
        )
    )
