...
SELECT 
    DISTINCT
    cast(epdd.proj_id as varchar(100)) AS project_id,
    cast(edhf.drl_hdr_no as integer),
    edhf.deliv_date AS required_date,
    cast(edhf.mat_all_run_no as integer),
    edhf.planner_usr_id AS entered_by,
    edhf.foreman AS requested_by,
    edhf.rem AS remarks,
    edhf.material_request_no,
    edhf.deliver_to,
    cast(edif.drl_item_id as integer) AS linked_to_line_no,
    cast(edif.mat_no as integer) AS item_no,
    edif.descr AS description,
    edif.drl_item_quan AS requested_qty,
    cast(edif.draw_no as integer) AS pmto_drawing_sheet,
    cast(edif.mto_pos as integer) AS pmto_pos,
    cast(edif.drl_item_no as integer),
    cast(edif.drl_hdr_no as integer) AS drl_hdr_no_item_fact,
    ...
    CASE                        
        ...
        ELSE NULL 
    END AS issue_status,
    cast(t1.rank as integer) as index,
...
FROM
    {{ source('supply_chain_dim', 'transformed_erm_delivery_header_fact') }} edhf
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_delivery_item_fact') }} edif ON edhf.drl_hdr_no = edif.drl_hdr_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_mat_grp_type_dim') }} emgtd ON edif.mat_grp_type_no = emgtd.mat_grp_type_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_pick_lst_stat_dim') }} eplsd ON edhf.pick_lst_stat_no = eplsd.pick_lst_stat_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_picking_lst_fact') }} eplf ON edhf.drl_hdr_no = eplf.drl_hdr_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_store_hdr_dim') }} eshd ON eplf.store_hdr_no = eshd.store_hdr_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_site_dim') }} esd ON edhf.site_no = esd.site_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_site_mto_hdr_dim') }} esmhd ON edhf.site_mto_hdr_no = esmhd.site_mto_hdr_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_suppl') }} es ON edhf.subcontractor_no = es.suppl_no
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_project_details_dim') }} epdd ON edhf.proj_no = epdd.proj_no
    LEFT JOIN(
        SELECT
        DISTINCT
        drl_hdr_no,
...
FROM
    {{ source('supply_chain_dim', 'transformed_erm_picking_lst_fact') }}) t2 ON edhf.drl_hdr_no = t2.drl_hdr_no
    LEFT JOIN (
        SELECT 
            DISTINCT
            drl_hdr_no,
        ...
        FROM 
            {{ source('supply_chain_dim', 'transformed_erm_delivery_header_fact') }}
        GROUP BY 
