...
{{
    config(
        ...
        custom_location=target.location ~ 'transformed_gsr_flexera/',
...
select DISTINCT gsr_id as gsr_id,
  case
  ...
  END as flexera_id,
  case
    when UPPER(flexera_installation_publisher) = 'NAN' then Null
    else flexera_installation_publisher
  end as flexera_installation_publisher,
  case
    when UPPER(flexera_installation_application_name) = 'NAN' then Null
    else flexera_installation_application_name
  end as flexera_installation_application_name,
  case
    when UPPER(flexera_application_classification) = 'NAN' then Null
    else flexera_application_classification
  end as flexera_application_classification,
  case
    when UPPER(flexera_inventory_device_inventory_device_name) = 'NAN' then Null
    else flexera_inventory_device_inventory_device_name
  end as flexera_inventory_device_inventory_device_name,
  case
    when UPPER(flexera_inventory_device_operating_system) = 'NAN' then Null
    else flexera_inventory_device_operating_system
  end as flexera_inventory_device_operating_system,
  case
    when UPPER(flexera_last_logged_on_user_account) = 'NAN' then Null
    else flexera_last_logged_on_user_account
  end as flexera_last_logged_on_user_account,
  case
    when UPPER(flexera_calculated_user_account) = 'NAN' then Null
    else flexera_calculated_user_account
  end as flexera_calculated_user_account,
  case
    when UPPER(flexera_inventory_device_inventory_device_type) = 'NAN' then Null
    else flexera_inventory_device_inventory_device_type
  end as flexera_inventory_device_inventory_device_type,
  case
    when UPPER(flexera_installation_version) = 'NAN' then Null
    else flexera_installation_version
  end as flexera_installation_version,
  case
    when UPPER(flexera_application_edition) = 'NAN' then Null
    else flexera_application_edition
  end as flexera_application_edition,
  case
    when UPPER(flexera_installation_is_used) = 'NAN' then Null
    else flexera_installation_is_used
  end as flexera_installation_is_used,
  case
    when UPPER(flexera_application_number_of_sessions_for_usage) = 'NAN' then Null
    else flexera_application_number_of_sessions_for_usage
  end as flexera_application_number_of_sessions_for_usage,
  case
    when UPPER(flexera_inventory_device_location) = 'NAN' then Null
    else flexera_inventory_device_location
  end as flexera_inventory_device_location,
  cast(
    case
      when UPPER(flexera_installation_last_used_date) IN ('','NAT') then Null
      else flexera_installation_last_used_date
    end as timestamp
  ) as flexera_installation_last_used_date,
  region_country_mapping_description,
  region_country_mapping_region,
  CASE
    WHEN gsr.execution_date IS NULL THEN NULL
    ELSE CAST(gsr.execution_date as TIMESTAMP)
  END AS execution_date,
  {{run_date}} as model_created_date,
  {{run_date}} as model_updated_date,
  {{ generate_load_id(model) }} as model_load_id
from {{ source('curated_hexagon','curated_entitlement_global_software_register') }} gsr
  join {{ source('curated_hexagon', 'curated_consumption_flexera') }} flex 
  on gsr.gsr_id = CASE
    -- Check if it starts with 'GSR', format accordingly
    WHEN LEFT(
      LEFT(
  ...
  on UPPER(SPLIT_PART(flexera_inventory_device_location, '/', 1)) = UPPER(loc.region_country_mapping_country)