...
mob_data as(select mob.customer_key, MAX(mob.dt) AS Last_Contacted_Date_Cox_App from mobile_data.app_contact_history mob group by mob.customer_key)
    SELECT distinct
...
mob_data as(
    select mob.customer_key, MAX(mob.dt) AS Last_Logged_In_Date_Cox_App from mobile_data.app_contact_history mob group by mob.customer_key
),
app_contact AS (
    SELECT
        m.customer_key,
    ...
    FROM
        mobile_data.app_contact_history m
    GROUP BY
        m.customer_key
),
...
LEFT JOIN 
mobile_data.app_contact_history a 
