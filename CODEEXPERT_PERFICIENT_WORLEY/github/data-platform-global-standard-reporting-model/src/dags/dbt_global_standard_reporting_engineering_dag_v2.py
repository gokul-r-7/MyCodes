...
payload = json.dumps({
          "schema": "engineering", 
          "obt_name": ["e3d_pipes_trending_obt","e3d_pipe_obt","e3d_cwp_obt","e3d_iso_stats_quantities",
          "e3d_pipe_latest_obt","e3d_pipe_spec_size_obt","e3d_pipe_std_support_obt","e3d_project_dim_obt",
          "e3d_roc_status_obt","e3d_wbs_index_obt","e3d_wbs_obt","e3d_equipment_obt","e3d_structural_quant_obt"], 
          "db": "global_standard_reporting",
          "dag_name": "dbt_global_standard_reporting_engineering_dag_v2"
          })
...
#documentcontrol_dataset = Dataset("//engineering/domain_integrated_model/global_standard_reporting")
def check_dbt_failures(**kwargs):
    if kwargs['ti'].state == 'failed':
        raise AirflowException('Failure in dbt task group')

with DAG(
    dag_id="dbt_global_standard_reporting_engineering_dag_v2",
    ...
    audit_dbt_task >> dbt_task >> dbt_check >> sns_notification_for_failure_for_modelling_task_group>> apply_redshift_permissions >> sns_notification_for_failure
