...
SELECT
    *,
    ...
    po_line_quantity * line_item_unit_price as usd_line_total_conv,
    line_item_unit_price as usd_line_unit_conv,
    CASE 
    when usd_line_total_conv<0.0001 then 1
    else 0 end as po_total_conv,
    CASE
    when po_line_quantity * line_item_unit_price< 10000 then '<10K'
    when po_line_quantity * line_item_unit_price >=10000 AND
    po_line_quantity * line_item_unit_price < 100000 then '10K-100K'
    when po_line_quantity * line_item_unit_price >=100000 AND
    po_line_quantity * line_item_unit_price <= 1000000 then '100K-1M'
...
FROM
    (
        SELECT
            ...
            ROUND(po_line_quantity*line_item_unit_price) as line_item_total_price,
            pol.promised_date,
...
FROM
    ...
    LEFT JOIN {{ source('supply_chain_dim', 'erm_po_line_item') }} pol ON poh.purchase_order_internal_id = pol.po_internal_id
    LEFT JOIN {{ source('supply_chain_dim', 'erm_po_reciepts') }} por ON pol.purchase_order_line_item_internal_id =
    por.po_line_item_internal_id
    LEFT JOIN {{ source('supply_chain_dim', 'erm_supplier_country') }} spc ON poh.supplier_code = spc.supplier_no
    LEFT JOIN {{ source('supply_chain_dim', 'erm_suppl') }} spl ON poh.supplier_code = spl.suppl_id
    INNER JOIN {{ source('supply_chain_dim', 'erm_dim_country') }} dic ON spl.country_id = dic.country_id
    LEFT JOIN {{ source('supply_chain_dim', 'erm_project_location') }} prol ON poh.project_number = prol.project_id
    LEFT JOIN {{ source('supply_chain_dim', 'cext_w_psr_st_api_dh') }} psr ON psr.po_number = poh.po_number
    LEFT JOIN {{ source('supply_chain_dim', 'erm_taxonomy_master') }} tam ON tam.unspsc = pol.financial_account_code
    )
