...
SELECT
    *,
    ...
    EXTRACT(year FROM received_date) AS received_year,
    EXTRACT(month FROM received_date) AS received_month_no, 
    'Qtr '|| EXTRACT(quarter FROM received_date) as received_quarter, 
    to_char(received_date , 'Mon')AS received_month,
    TO_NUMBER(TO_CHAR(received_date,'YYYYMMDD'),'99999999') as received_date_key,
    EXTRACT(year FROM po_creation_date) AS po_creation_year,
    EXTRACT(month FROM po_creation_date) AS po_creation_month_no, 
    'Qtr '|| EXTRACT(quarter FROM po_creation_date) as po_creation_quarter, 
    TO_CHAR(po_creation_date, 'Mon') AS po_creation_month,
    TO_NUMBER(TO_CHAR(po_creation_date,'YYYYMMDD'),'99999999') as po_creation_date_key,
    EXTRACT(year FROM po_issue_date) AS po_issue_year,
    EXTRACT(month FROM po_issue_date) AS po_issue_month_no, 
    'Qtr '|| EXTRACT(quarter FROM po_issue_date) as po_issue_quarter, 
    to_char(po_issue_date , 'Mon') AS po_issue_month,
    ...
    when po_line_quantity * line_item_unit_price*usd_conv >=10000 AND
    po_line_quantity * line_item_unit_price*usd_conv < 100000 then '10K-100K'
    when po_line_quantity * line_item_unit_price*usd_conv >=100000 AND
    po_line_quantity * line_item_unit_price*usd_conv <= 1000000 then '100K-1M'
    else '>1M' end as po_range_filter,
    po_number||'_'||po_line_number as po_and_line_item_no,
    'Refreshed Date: '+TO_CHAR(TO_DATE(data_refreshed_date, 'YYYY-MM-DD'), 'DD-Mon-YYYY') AS refreshed_date,
    GETDATE() as dag_execution_date
   
FROM
    (
        SELECT DISTINCT
            cast(poh.project_number as varchar(100)) as project_id,
            poh.po_number,
            ...
            CASE when
            tam.unspsc is null then '00 Not Classified' else tam.unspsc end as unspsc,
            ...
            tam.commodity_l5,
            CASE when
            psr.sub_project = '' then NULL else sub_project end as sub_project,
            CASE when 
            tam.category_l2 is null then '00 Not Classified' else tam.category_l2 end as ctg_category_l2 ,
            CASE when
            tam.sub_category_l3 is null then '00 Not Classified' else tam.sub_category_l3 end as ctg_sub_category_l3 ,
            CASE when
            tam.family_l4 is null then '00 Not Classified' else tam.family_l4 end as ctg_family_l4 ,
            CASE when
            tam.commodity_l5 is null then '00 Not Classified' else tam.commodity_l5 end as ctg_commodity_l5               
FROM
    {{ source('supply_chain_dim', 'transformed_erm_po_header') }} poh
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_po_line_item') }} pol ON poh.purchase_order_internal_id = pol.po_internal_id
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_po_reciepts') }} por ON pol.purchase_order_line_item_internal_id =
    por.po_line_item_internal_id
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_suppl') }} spl ON poh.supplier_code = spl.suppl_id
    INNER JOIN {{ source('supply_chain_dim', 'transformed_erm_dim_country') }} dic ON spl.country_id = dic.country_id
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_project_location') }} prol ON poh.project_number = prol.project_id
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_cext_w_psr_st_api_dh') }} psr ON psr.po_number = poh.po_number
    LEFT JOIN {{ source('supply_chain_dim', 'transformed_erm_taxonomy_master') }} tam ON tam.unspsc = pol.financial_account_code
    LEFT JOIN (
                        with currency_exchange_rate as (
                            ...
                            from
                                {{ source('finance', 'transformed_currency_exchange_rate') }}
                        )
                        SELECT
                            from_currency,
                            to_currency,
