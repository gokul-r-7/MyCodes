...
initial_sourcing = ["secorglist","snapidlist","costtypepcsmapping","users","calendar"]
secorg_datasets = ["header","snapshotlog","snapshotfull","findata","findatapcs","archival"]
...
glue_job_names = {
    "api_sourcing_job": f"worley-datalake-sydney-{environment}-glue-job-ecosys-workflow-api-sourcing",
...
# Create DAG
with DAG(
    dag_id=f"ecosys_{environment}_generic_data_pipeline",
    schedule="@once",
    ...
    raw_crawler >> detect_schema_change() >> raw_to_curated() >> end_task
    
    #(
    #    ecosys_intial_sourcing.expand(metadata=initial_sourcing) >> ecosys_root_secorg_list_task >> [ecosys_entity_sourcing.expand(secorg_id=ecosys_root_secorg_list_task) >> ecosys_ecodata.expand(secorg_id=ecosys_root_secorg_list_task)  >> ecosys_other_sourcing()] >> raw_crawler >> detect_schema_change() >> raw_to_curated()
    #)
