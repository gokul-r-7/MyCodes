...
def get_sharepoint_item_list(sharepoint_id,document_library_name):
    logger.info(f"Get data for {document_library_name}")
    sharepoint_item_list=[]
    url = f"{graph_url}/{sharepoint_id}/lists/{document_library_name}/items?$expand=fields&select=id,webUrl,fields/ContentType,fields/LinkFilename,fields/DocIcon,createdDateTime,lastModifiedDateTime"
    print(url)
...
def download_sharepoint_files(filelist):
    ...
    for file_info in filelist:
        ...
        #s3_file_name=f'{s3_root_folder}/{incremental_folder_name}/{fileName}'
        s3_file_name=f'{s3_root_folder}/{fileName}'
...
try:
    ...
    if sharepoint_site_id is not None:
        ...
        sharepointlist=get_sharepoint_item_list(sharepoint_site_id,sharepointDocumentLibrary)
    else:
