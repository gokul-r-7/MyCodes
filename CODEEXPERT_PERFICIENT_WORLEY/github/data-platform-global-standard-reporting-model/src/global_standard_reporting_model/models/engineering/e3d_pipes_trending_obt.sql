...
select *, 
cast(CASE when Progress = 0 then 0 
     when Line_count = 0 then 0
     else  (Progress/Line_count) 
     END as decimal(38,4)) as Line_progress_perc,
cast(Case when progress = 0 then 0
     when iso_count= 0 then 0
     else (progress/iso_count)
     END as decimal(38,4)) as iso_progress_perc,
...
cast(sum(progress) as decimal(38,2))  as Progress,
