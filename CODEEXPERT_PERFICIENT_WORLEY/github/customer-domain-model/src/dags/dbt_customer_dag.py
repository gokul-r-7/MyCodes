...
csp_salesforce_dataset = Dataset("//csp_salesforce/customer/domain_integrated_model")
...
SECRET_NAME = dag_config["customer"]
...
#         conn_id="redshift_customer_connection",
#         profile_args={"schema": "domain_integrated_model", "user": username, "password": password}
#     ),
# )

DBT_PROJECT_PATH = os.environ.get("DBT_CUSTOMER_PROJECT_PATH", "/usr/local/airflow/dags/dbt").strip()
...
payload = json.dumps({
          "dag_name": "dbt_customer_dag",
          "domain": "circuit_breaker"
          })

with DAG(
    dag_id="dbt_customer_dag",
    start_date=datetime(2025, 1, 1),
    schedule=[csp_salesforce_dataset],
    catchup=False,
    tags=["dbt"],
):
    ...
    customer_dbt_task = DbtTaskGroup(
        group_id="customer_dbt_task",
        execution_config=execution_config,
        profile_config=profile_config,
        project_config=project_config,
        operator_args={
    ...
    audit_dbt_task >>  customer_dbt_task >> dbt_check >> sns_notification_for_failure_for_modelling_task_group >> apply_redshift_permissions >> sns_notification_for_failure_for_other_tasks
    
    # [customer_dbt_task, apply_redshift_permissions] >> customer_run_gsr_models_dag
