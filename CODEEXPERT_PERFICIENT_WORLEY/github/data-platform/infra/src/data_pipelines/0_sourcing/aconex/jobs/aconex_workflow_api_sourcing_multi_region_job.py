...
spark_conf = SparkConf().setAll([
  ...
  ("spark.hadoop.fs.s3.serverSideEncryption.kms.keyId", "mrk-3c56ea42a1144fae90bd5551d5e7d543")
])
...
#        if column_name in workflow_df.columns:
        ...
        combined_df = combined_df.withColumn("Assignee_name",when(col("id").isNull(), col("Assignee_main_Name")).otherwise(col("Assignee_array_Name")))
        combined_df = combined_df.withColumn("Assignee_OrganizationName",when(col("id").isNull(), col("Assignee_main_OrganizationName")).otherwise(col("Assignee_array_OrganizationName")))
        combined_df = combined_df.withColumn("Assignee_OrganizationId",when(col("id").isNull(), col("Assignee_main_OrganizationId")).otherwise(col("Assignee_array_OrganizationId")))
        combined_df = combined_df.withColumn("Assignee_UserId",when(col("id").isNull(), col("Assignee_main_UserId")).otherwise(col("Assignee_array_UserId")))
