...
relationalized_data_path = f"{relationalized_data_path}/{instance_name}/relationalized_data"
...
temp_path=f"s3://{bucket_name}/{temp_deletion_path}/{instance_name}/"
...
try:
  endpoint_url= endpoint + "directory?org_name=" +org_name
  print("endpoint_url --> " + endpoint_url)
  aconex_export_config['api_parameter']['endpoint']=f"{endpoint_url}"
  http_client = HTTPClient(aconex_export_config)
  User_Directory_response, api_status,status_code = http_client.run()
  print(User_Directory_response)
  if User_Directory_response:
    object_name = f"{raw_data_location}/{instance_name}/data.xml" 
    if s3_client.upload_to_s3(User_Directory_response,object_name,kms_key_id,is_gzip=True):
     ...
     success = True
      ...
      success = False 
    root=ET.fromstring(User_Directory_response)
  else:
...
try:
  if success:
    ...
    root_df = glueContext.create_dynamic_frame.from_options(
            ...
            connection_options={
                "paths": [
                    f"s3://{bucket_name}/{raw_data_location}/{instance_name}/"
                    ],
                    "recurse": True
                },
                format="xml",
