...
roc,
progress,
cast((progress/100) as decimal(38,4)) as progress_perc,
holdflag,
"zone",
site,
wbs,
...
is_active,
dag_execution_date,
cast(SUM(length1) as decimal(38,4)) as length_inch,
case when length_inch = 0.0 then 0.0 else CAST((length_inch/12) as numeric(38,4)) END as thru_length_ft,
cast(SUM(tube_length1) as decimal(38,4))  as tube_length_inch,
case when tube_length_inch = 0.0 then 0.0 else cast((tube_length_inch/12) as numeric(38,4)) END AS tube_length_ft,
...
cast(case when W.Id is NULL THEN 0 else W.Id END as integer) as wbs_id,
P.extracted_date,
...
FROM  {{ source('engineering_dim', 'transformed_e3d_pipes') }} where wbs <> ''
...
(select top 90 distinct extracted_date from {{ source('engineering_dim', 'transformed_e3d_pipes') }} 
