...
def find_matching_file_in_s3_bucket(filename, file_prefix=0, file_suffix=0):
    try:
        ...
        logger.info(f"These are the list of files in S3 {response}")
...
def convert_xlsx_to_parquet(xls_filename, sheet, xls_engine_name, is_fixed_file_name, file_prefix, file_suffix):
    try:
        ...
        logger.info(f"skiprows: {skiprows}")
        ...
        df = read_excel_from_s3(bucket_name, f"{raw_files_path}{xls_filename}", sheet_name, header_row, xls_engine_name, metadata["aws_region"], skiprows)
        ...
        new_df.columns = df.columns[header_column:].str.lower()
