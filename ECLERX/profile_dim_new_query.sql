...
mfa_data AS (
    ...
    GROUP BY
        a.user_id, c.eventtype, c.event_date,DATE_FORMAT(CAST(o.event_date AS TIMESTAMP), '%Y-%m')
),
account_details AS (
...
AND DATE_FORMAT(CAST(r.time_key AS TIMESTAMP), '%Y-%m') >= mob.Contact_Month and mob.rn=1
...
left JOIN  mfa_data mfa ON a.user_id = mfa.user_id AND DATE_FORMAT(CAST(r.time_key AS TIMESTAMP), '%Y-%m') >= mfa.Contact_Month
