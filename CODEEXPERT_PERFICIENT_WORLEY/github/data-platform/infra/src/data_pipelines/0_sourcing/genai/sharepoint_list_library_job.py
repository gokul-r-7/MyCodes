...
def get_sharepoint_item_list(sharepointsite_name,list_library_name,sharepointListLibraryQueryParam):
    logger.info(f"Get data for {list_library_name} in {sharepointsite_name} with param {sharepointListLibraryQueryParam}")
    sharepoint_item_list=[]
    sharepoint_url = f"https://{sharepointsite_name}/_api/web/lists/getbytitle('{list_library_name}')/items?{sharepointListLibraryQueryParam}"       
    page_number=1
    while sharepoint_url:
        ...
        if api_resp_code == 200:
            ...
            sharepoint_item_list.extend([
                {
                    **item,
                    "AttachmentFiles": attachment
                }
                for item in response.get("value", [])
                for attachment in item.get("AttachmentFiles", [])
...
def clean_sharepoint_list(sharepointData_list,api_fields_to_remove):
    ...
    for item in sharepointData_list:
        file_name=item.get("AttachmentFiles", {}).get("FileName")
        relative_file_path=item.get("AttachmentFiles", {}).get("ServerRelativePath",{}).get("DecodedUrl")
    ...
    fields_to_remove=["FileLeafRef","FileRef","Id","ID","Created","Modified","odata.type","odata.id", "odata.etag"
                            ...
                            "EditorId","OData__CopySource","CheckoutUserId","OData__UIVersionString","GUID","SharedWithUsersId","SharedWithDetails","AttachmentFiles","AttachmentFiles@odata.navigationLinkUrl"]
...
def filter_files(filelist, file_filters):
...
sharepointListLibrary=sharepoint_config['job_parameter']['sharepointListLibrary']
sharepointListLibraryQueryParam=sharepoint_config['job_parameter']['sharepointListLibraryQueryParam']
...
try:
    ...
    if sharepointListLibrary is not None:
    ...
    logger.info(f"Getting items from Sharepoint Site Id  {sharepointsiteurl} for Library {sharepointListLibrary}")
    sharepoint_item_list=get_sharepoint_item_list(sharepointsiteurl,sharepointListLibrary,sharepointListLibraryQueryParam)
    ...
    delta_filelist=filter_files(sharepoint_item_list,file_filters)
