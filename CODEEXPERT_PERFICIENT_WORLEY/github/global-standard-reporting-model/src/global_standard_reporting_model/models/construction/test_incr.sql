...
{{
     config(
         ...
         tags=["construction"],
         unique_key = "comp_hours_key",
         incremental_strategy = "merge",
         merge_exclude_columns = ['r_created_date'],
...
with changehistory_table as 
(
select 
projectid,entityname,cast(datecreated as date) datecreated,max(updated_date) as updated_date,cast(max(newvalue) as numeric(9,2)) newvalue,
...
where fieldname= 'Earned Hours'  and  cast(newvalue as decimal(9,2))>0 and Cast(datecreated as date) is not null 
 {%- if execution_date_arg != "" %}
    ...
    {%- else %}
        {%- if is_incremental() %}
            and cast(updated_date as DATE) > (select max(ch_load_date) from {{ this }})
        {%- endif %}
    {%- endif %}
...
obt_result as (
select 
change_history.projectid|| '-' || change_history.entityname || '-' || cwp.cwpsid || '-' || change_history.datecreated comp_hours_key,
...
cast(change_history.updated_date as date) as ch_load_date
...
inner join changehistory_table change_history on iwp.iwpsid=change_history.entityname
...
iwp_updated_date,
ch_load_date,
getdate() as r_created_date,
getdate() as r_updated_date,
