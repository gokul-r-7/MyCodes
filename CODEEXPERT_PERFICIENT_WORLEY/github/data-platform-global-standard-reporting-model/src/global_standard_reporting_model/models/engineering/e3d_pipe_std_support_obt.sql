...
FROM  {{ source('engineering_dim', 'transformed_e3d_pipe_support_attr') }} as P
left join (
        with MaterialsCTE AS(
          ...
          from
               {{ source('engineering_dim', 'transformed_e3d_pipes_mto_comps') }}
          where
               extracted_date = (select max(extracted_date) FROM {{ source('engineering_dim', 'transformed_e3d_pipes_mto_comps') }} )
               and type = 'TUBI'
        )
         SELECT
                            pipe_ws,
...
where 
P.extracted_date = (select max(extracted_date) FROM {{ source('engineering_dim', 'transformed_e3d_pipe_support_attr') }})