...
{{
    config(
        ...
        custom_location=target.location ~ 'transformed_people/',
...
SELECT
    CAST(userid AS VARCHAR(100)) AS user_sid
    ,email AS user_email
    ,username AS display_name
    ,organizationname AS organization
    ,CAST(projectid AS VARCHAR(100)) AS dim_project_rls_key           
...
WHERE is_current = 1 AND email != 'NULL'
