...
for col_name, dtype in day_agg_df.dtypes:
    if dtype == 'void':
        day_agg_df = day_agg_df.withColumn(col_name, F.lit("").cast("string"))
day_agg_df.printSchema()
day_agg_df.count()
user_agg = """
WITH filtered_auth AS (
...
account_dim_df = account_dim_df.withColumn("time_key", F.to_date(account_dim_df["time_key"], "yyyy-MM-dd"))
account_dim_df.printSchema()
...
LEFT JOIN dwelling_data d ON r.dwelling_type_key = d.dwelling_type_key
"""
account_dim_13_df = spark.sql(account_dim_13)
account_dim_13_df.show()
account_dim_13_df.cache()
account_dim_13_df.printSchema()
account_dim_13_df = account_dim_13_df.withColumn("time_key", F.to_date(account_dim_13_df["time_key"], "yyyy-MM-dd"))
account_dim_13_df.printSchema()
account_dim_13_df.count()
profile_dim_sum_13 = """
WITH customer_dim AS (
    SELECT 
        b.customer_key AS Customer_Key,
...
LEFT JOIN notification_flags nf ON s.account_nbr = nf.customer_number
"""
profile_dim_sum_13_df = spark.sql(profile_dim_sum_13)
profile_dim_sum_13_df.show()
profile_dim_sum_13_df.cache()
profile_dim_sum_13_df.printSchema()
profile_dim_sum_13_df = profile_dim_sum_13_df.withColumn("time_key", F.to_date(profile_dim_sum_13_df["time_key"], "yyyy-MM-dd"))
profile_dim_sum_13_df.printSchema()
profile_dim_sum_13_df.count()
job.commit()