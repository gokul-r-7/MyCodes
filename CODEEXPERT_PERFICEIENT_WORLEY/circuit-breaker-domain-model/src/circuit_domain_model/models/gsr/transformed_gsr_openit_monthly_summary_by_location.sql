...
{{
    config(
        ...
        custom_location=target.location ~ 'transformed_gsr_openit_monthly_summary_by_location/',
...
select DISTINCT CASE
    WHEN POSITION(
      'GSR' IN openit.openit_monthly_summary_by_location_applicationname
    ) > 0 THEN 'GSR' || RIGHT(
      '0000' || SUBSTRING(
        openit.openit_monthly_summary_by_location_applicationname,
        POSITION(
          'GSR' IN openit.openit_monthly_summary_by_location_applicationname
        ) + 3,
        LENGTH(
    ...
    ELSE NULL
  END as openit_monthly_summary_by_location_id,
    ROUND(
        CAST(
            CASE
                WHEN openit_monthly_summary_by_location_maxavailable = 0 THEN NULL
                WHEN openit_monthly_summary_by_location_maxinuse IS NULL OR openit_monthly_summary_by_location_maxavailable IS NULL THEN NULL
                WHEN openit_monthly_summary_by_location_maxinuse = '' OR openit_monthly_summary_by_location_maxavailable = '' THEN NULL
                --WHEN NOT openit_monthly_summary_by_location_maxinuse ~ '^[0-9]+(\.[0-9]*)?$' THEN NULL
                WHEN NOT openit_monthly_summary_by_location_maxinuse rlike '^[0-9]+(\\.[0-9]*)?$' THEN NULL
                WHEN NOT openit_monthly_summary_by_location_maxavailable rlike '^[0-9]+(\.[0-9]*)?$' THEN NULL
                ELSE 

                    CAST(openit_monthly_summary_by_location_maxinuse AS DECIMAL(18,9))
                    /CAST(openit_monthly_summary_by_location_maxavailable AS DECIMAL(18,9))
                    --openit_monthly_summary_by_location_maxinuse::NUMERIC / openit_monthly_summary_by_location_maxavailable::NUMERIC
            END AS NUMERIC(18, 9)
        ),
        2
    ) AS maxutilization,
  LEFT(
    openit_monthly_summary_by_location_applicationname,
    POSITION(
      'GSR' IN openit_monthly_summary_by_location_applicationname
    ) - 1
  ) AS openit_monthly_summary_by_location_applicationname,
  case
    when UPPER(
      openit_monthly_summary_by_location_applicationversion
    ) = 'NAN' then Null
    else openit_monthly_summary_by_location_applicationversion
  end as openit_monthly_summary_by_location_applicationversion,
  case
    when UPPER(openit_monthly_summary_by_location_year) = 'NAN' then Null
    else openit_monthly_summary_by_location_year
  end as openit_monthly_summary_by_location_year,
  case
    when UPPER(openit_monthly_summary_by_location_month) = 'NAN' then Null
    else openit_monthly_summary_by_location_month
  end as openit_monthly_summary_by_location_month,
  case
    when UPPER(openit_monthly_summary_by_location_country) = 'NAN' then Null
    else openit_monthly_summary_by_location_country
  end as openit_monthly_summary_by_location_country,
  case
    when UPPER(openit_monthly_summary_by_location_elapsedtime) = 'NAN' then Null
    else openit_monthly_summary_by_location_elapsedtime
  end as openit_monthly_summary_by_location_elapsedtime,
  case
    when UPPER(openit_monthly_summary_by_location_maxinuse) = 'NAN' then Null
    else openit_monthly_summary_by_location_maxinuse
  end as openit_monthly_summary_by_location_maxinuse,
  case
    when UPPER(openit_monthly_summary_by_location_maxavailable) = 'NAN' then Null
    else openit_monthly_summary_by_location_maxavailable
  end as openit_monthly_summary_by_location_maxavailable,
  case
    when UPPER(openit_monthly_summary_by_location_denials) = 'NAN' then Null
    else openit_monthly_summary_by_location_denials
  end as openit_monthly_summary_by_location_denials,
  case
    when UPPER(openit_monthly_summary_by_location_distinctuser) = 'NAN' then Null
    else openit_monthly_summary_by_location_distinctuser
  end as openit_monthly_summary_by_location_distinctuser,
  case
    when UPPER(openit_monthly_summary_by_location_distincthost) = 'NAN' then Null
    else openit_monthly_summary_by_location_distincthost
  end as openit_monthly_summary_by_location_distincthost,
  gsr_id,
  case
  ...
  CAST(
    openit_monthly_summary_by_location_year || '-' || CASE
      openit_monthly_summary_by_location_month
      WHEN 'Jan' THEN '01'
      WHEN 'Feb' THEN '02'
      WHEN 'Mar' THEN '03'
      WHEN 'Apr' THEN '04'
      ...
      WHEN 'Dec' THEN '12'
    END || '-01' AS date
  ) AS openit_monthly_summary_by_location_transformed_date,
  COALESCE(
    loc.region_country_mapping_description,
    map.region_country_mapping_description
  ) as region_country_mapping_description,
  COALESCE(
    loc.region_country_mapping_region,
    map.region_country_mapping_region
  ) as region_country_mapping_region,
  CASE
...
from {{ source('curated_hexagon','curated_consumption_openit_monthly_summary_by_location') }} openit
  join {{ source('curated_hexagon','curated_entitlement_global_software_register') }} gsr on CASE
    WHEN POSITION(
      'GSR' IN openit.openit_monthly_summary_by_location_applicationname
    ) > 0 THEN 'GSR' || RIGHT(
      '0000' || SUBSTRING(
    ...
    ELSE NULL
  END = gsr.gsr_id
  left join {{ source('curated_hexagon','curated_consumption_region_country_mapping') }} loc 
  on UPPER(openit.openit_monthly_summary_by_location_country) = UPPER(loc.region_country_mapping_country)
  left join {{ source('curated_hexagon','curated_consumption_region_country_mapping') }} map 
  on UPPER(openit.openit_monthly_summary_by_location_country) = UPPER(map.region_country_mapping_description)