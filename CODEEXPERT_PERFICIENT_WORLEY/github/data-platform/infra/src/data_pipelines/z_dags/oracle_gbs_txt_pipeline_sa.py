...
datasets = ["gbs_suppliers", "gbs_ap_po"]
...
payload = json.dumps({
          "dag_name": f"oracle_gbs_txt_{environment}_sa_data_pipeline"
          })
...
# Create DAG
with DAG(
    dag_id=f"oracle_gbs_txt_{environment}_sa_data_pipeline",
    schedule="@once",
    tags=["oracle_gbs_txt"],
    ...
    sftp_txt_file = GlueJobOperator(
        task_id="sftp_txt_file",
        ...
        script_args={
            ...
            "--bucket_key": 'finance/oracle_gbs/raw/EBSCR9/'+'{{ ti.xcom_pull(task_ids="partition_path") }}',
            ...
            "--function_name": txt_file_function_name
        }
    )
    ...
    @task_group(group_id="oracle_gbs_txt_csv_sourcing")
    def oracle_gbs_txt_csv_sourcing():
        """Task Group that runs the required steps to source oracle_gbs"""

        txt_csv_task = GlueJobOperator(
            ...
            script_args={
                "--source_name": source_system_id,
                "--metadata_type": f"csv_txt#{source_system_id}",
                "--function_name": "csv_txt",
                "--start_date": execution_date,
                "--end_date": execution_date,
                "--connector_file_path" : 'finance/oracle_gbs/raw/EBSCR9/'+'{{ ti.xcom_pull(task_ids="partition_path") }}'
            },
        )

        txt_csv_task
        
    sourcing_txt_csv_tasks = oracle_gbs_txt_csv_sourcing()
    ...
    s3_partition_path >> sftp_txt_file >> sourcing_txt_csv_tasks >> raw_crawler >> detect_schema_change() >> raw_to_curated() >> run_finance_models_dag >> sns_notification_for_failure
