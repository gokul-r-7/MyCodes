...
endpoint_host ="ca1.aconex.com"
instance_name= "ca_instance"
...
with DAG(
    ...
    @task_group(group_id="run_ca_aconex_users_glue_jobs")
    def run_ca_users_glue_jobs(ProjectId):
        """Task Group to run all Glue jobs in parallel for a given project ID"""
        glue_job_tasks = []

        # User Project Task
        user_project_task = GlueJobOperator(
                task_id=f"aconex_ca_user_project",
        ...
        user_projectrole_task = GlueJobOperator(
                task_id=f"aconex_ca_user_projectrole",
    ...
    UserDirectory_job = GlueJobOperator(
        task_id="aconex_ca_userDirectory",
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"aconex_aconex_ca_raw_crawler",
        config={
    ...
    aconex_project_list_task >> run_ca_users_glue_jobs.expand(ProjectId=aconex_project_list_task) >> raw_crawler >> raw_crawler_sensor
