...
gsr_dataset = Dataset("//project_control/domain_integrated_model/global_standard_reporting")
...
secret_name = dag_config["project_control"]

payload = json.dumps({
          "dag_name": "dbt_project_control_ecosys_dag",
...
#     target_name="project_control",
#     profile_mapping=RedshiftUserPasswordProfileMapping(
#         conn_id="redshift_project_control_connection",
#         profile_args={"schema": "domain_integrated_model", "user": username, "password": password}
#     ),
# )
...
with DAG(
    dag_id="dbt_project_control_v2_ecosys_dag",
    start_date=datetime(2024, 9, 18),
    schedule=[project_control_dataset],
    catchup=False,
    tags=["dbt"],
):
    ...
    dbt_task = DbtTaskGroup(
        group_id="project_control_v2_ecosys_dbt_task",
        execution_config=execution_config,
        profile_config=profile_config,
        project_config=project_config,
        operator_args={
        ...
        render_config= RenderConfig(
            exclude=["tag:audit","tag:snowflake_models","tag:p6", "tag:v1"],
            select=["tag:ecosys","tag:v2"],
            load_method=LoadMode.DBT_LS
        )
    )
    ...
    audit_dbt_task >> dbt_task >> dbt_check >> sns_notification_for_failure_for_modelling_task_group >> apply_redshift_permissions >> run_gsr_models_dag >> sns_notification_for_failure_for_other_tasks
