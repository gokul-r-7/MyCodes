...
customer_status As
(select distinct customer_substatus_key,sub_status_desc from edw.customer_substatus_dim),
dwelling_data AS (
...
account_details AS (
    ...
    FROM edw.customer_dim a left join camp_mgmt.accounts ac on a.account_nbr = ac.account_nbr
    left JOIN camp_mgmt.email_addresses em ON em.vndr_cust_account_key = ac.vndr_cust_account_key
),
notification_flags AS (
    SELECT distinct a.customer_key,
        MAX(CASE WHEN notification_method = 'EMAIL' THEN 'Y' ELSE 'N' END) AS Notification_Email_Flag,
        MAX(CASE WHEN notification_method = 'PHONE' THEN 'Y' ELSE 'N' END) AS Notification_Phone_Flag,
    ...
    FROM edw.customer_dim a left join pstage.ALL_CUST_NOTIFICATION_METHODS n on a.account_nbr=n.customer_number
...
app_contact AS (
    ...
    FROM mobile_data.app_contact_history
    WHERE visits IN (
        SELECT DISTINCT visits
...
mob_data AS (
    ...
    FROM mobile_data.app_contact_history mob
...
revenue_data AS (
    SELECT r.customer_key,
		   ...
		   r.time_key
    FROM edw.customer_revenue_fact r LEFT JOIN (SELECT DISTINCT
		a.Customer_Key,
     ...
     FROM
        edw.customer_guid_dtl_dim a LEFT JOIN ciam.successful_authentications_okta o on a.user_id = o.actor_alternateid
        group by
        a.Customer_Key,a.household_member_guid,DATE_FORMAT(CAST(o.event_date AS DATE), '%Y-%m'))o ON r.customer_key=o.customer_key and o.Contact_Month <= r.time_key
	 WHERE r.bill_type_key != 2 and
    DATE_PARSE(r.time_key, '%Y-%m-%d') >= DATE_ADD('month', -13, CURRENT_DATE)
...
mob_data AS (
    ...
    FROM
        mobile_data.app_contact_history mob
)
...
WITH web_contact_history AS (
    ...
    FROM webanalytics.web_contact_history
    WHERE DATE_PARSE(SUBSTR(CAST(dt AS varchar), 1, 19), '%Y-%m-%d') >= DATE_ADD('day', -30, CURRENT_DATE)
...
SELECT
    ...
    cast(NULL as varchar) AS Authentication_Error,
...
LEFT JOIN
    aggregated_signon o ON  o.user_id=g.user_id
LEFT JOIN
    ciam.mfa_total_mfa_users m ON m.username=g.user_id
group by 
    g.household_member_guid,
	a.host,
    o.application,
