...
datasets = ["project_customer_list","project_key_member_list","project_classification_list","df_person"]
...
payload = json.dumps({
          "dag_name": f"oracle_gbs_r4_csv_{environment}_data_pipeline"
          })
...
# Create DAG
with DAG(
    dag_id=f"oracle_gbs_r4_csv_{environment}_data_pipeline",
    schedule="30 7 * * 1",
    tags=["oracle_gbs_r4_csv"],
    ...
    sftp_sourcing = GlueJobOperator(
        ...
        script_args={
            ...
            "--bucket_key": 'finance/oracle_gbs/raw/r4/'+'{{ ti.xcom_pull(task_ids="partition_path") }}',
    ...
    @task_group(group_id="oracle_gbs_r4_csv_sourcing")
    def oracle_gbs_r4_csv_sourcing():
        ...
        csv_task = GlueJobOperator(
            ...
            script_args={
                "--source_name": source_system_id,
                "--metadata_type": f"csv_r4#{source_system_id}",
                "--function_name": "csv_r4",
                "--start_date": execution_date,
                "--end_date": execution_date,
                "--connector_file_path" : 'finance/oracle_gbs/raw/r4/'+'{{ ti.xcom_pull(task_ids="partition_path") }}'
            },
        )

        csv_task
        
    sourcing_tasks = oracle_gbs_r4_csv_sourcing()
    
   

    raw_crawler = GlueCrawlerOperator(
        task_id=f"oracle_gbs_csv_r4_raw_crawler",
        config={
    ...
    s3_partition_path >> sftp_sourcing >> sourcing_tasks >> raw_crawler >> detect_schema_change() >> raw_to_curated() >> run_finance_models_dag >> sns_notification_for_failure
