...
projects = [str(d) for d in metadata[environment]['Project_DAC']
...
# Create DAG
with DAG(
    dag_id=f"o3_{environment}_generic_data_pipeline_dac",
    schedule="0 0 * * *",
    ...
    o3_intial_sourcing.expand(project_id=projects) >> raw_crawler >> raw_crawler_sensor >> detect_schema_change() >> raw_to_curated() >> run_construction_models_dag
