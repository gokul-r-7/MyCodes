...
SELECT
    *,
    CASE
    WHEN "Status with Closed" <> 'Closed' then 'Open'
    ELSE "Status with Closed" end as "Open/Close",
    CASE
    WHEN status <> 'Paid' AND payment_due_date < CURRENT_DATE AND payment_due_date IS NOT NULL THEN
            ...
            END
        ELSE status
    END AS "Status with Past Due"
FROM(
    SELECT DISTINCT
        proj.project_code as project_id,
        ...
        ebu.business_units,
        CASE
        when com."%po_complete" > 100 then 'Over Paid'
        else 'Compliant' end as "Overpaid or Complaint",
        CASE
        when com.po_close_date is null THEN (
            CASE
            when com."%po_complete" is null then 'In Progress'
            when com."%po_complete" = 100 then 'Paid/Delivered Ready TO Close'
            when com."%po_complete" > 100 then 'Overpaid PO'
            else 'In Progress' end
        )
        else 'Closed' end as "Status with Closed",
        ...
        inv.dataset_refershed_date
    FROM
        {{ ref('fact_po_invoices_obt') }} inv
        LEFT JOIN {{ ref('fact_po_commitment_obt') }} com ON inv.po_number = com.po_number
        LEFT JOIN {{ source('supply_chain_dim', 'erm_suppl') }} sup ON inv.supplier_no = sup.suppl_id
        LEFT JOIN {{ source('supply_chain_dim', 'erm_project_dim') }} proj ON com.project_id = proj.project_code
        LEFT JOIN {{ source('supply_chain_dim', 'erm_business_unit') }} ebu ON com.project_id = ebu.project
)