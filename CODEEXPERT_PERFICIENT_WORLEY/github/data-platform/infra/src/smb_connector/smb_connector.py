...
def process_source(source, smb_host, username, password, s3_helper, partition_str, batch_run_start_time, job_config):
    ...
    try:
        ...
        for pattern in patterns:
            ...
            files = smb_file_listor.list_files()  # Get files for the current pattern
        ...
        s3_key = f"{bucket_key}/{batch_run_start_time}/"
    ...
    try:
        with SMBConnection(
            ...
            for source in job_config.smb_configuration.source:
                ...
                username,                               
                password,                               
                s3_helper, partition_str, batch_run_start_time, job_config
            )
