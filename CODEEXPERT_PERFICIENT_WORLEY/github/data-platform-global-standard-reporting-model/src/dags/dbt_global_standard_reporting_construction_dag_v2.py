...
payload = json.dumps({
          ...
          "dag_name": "dbt_global_standard_reporting_construction_dag_v2"
          })
...
#documentcontrol_dataset = Dataset("//construction/domain_integrated_model/global_standard_reporting")
def check_dbt_failures(**kwargs):
    if kwargs['ti'].state == 'failed':
        raise AirflowException('Failure in dbt task group')
    
with DAG(
    dag_id="dbt_global_standard_reporting_construction_dag_v2",
