...
WITH customer_data AS (
    ...
    WHERE  c.site_id = 186 and test_account_key=2
),
revenue_data AS (
...
account_summary AS (
    ...
    WHERE s.site_id = 186 AND s.CUSTOMER_TYPE_CD IN ('1', 'R') and 
...
SELECT
    ...
    COUNT(a.uuid) AS Authentication_Attempt, 
    COUNT(DISTINCT CASE WHEN a.outcome_result = 'SUCCESS' THEN a.uuid END) AS authentication_success_result,
...
INNER JOIN
    edw.customer_dim c ON g.customer_key = c.customer_key AND c.site_id = 186
LEFT JOIN
    ciam.successful_authentications_okta a ON g.user_id = a.actor_alternateid
LEFT JOIN
    ciam.single_signon_okta o ON g.user_id = o.username
LEFT JOIN
    ciam.mfa_total_mfa_users m ON g.user_id = m.username
WHERE
    CAST(a.event_date AS DATE) >= DATE_ADD('day', -90, CURRENT_DATE)
    AND CAST(o.event_date AS DATE) >= DATE_ADD('day', -90, CURRENT_DATE)
GROUP BY
    g.household_member_guid,
    a.host,
    o.application,
...
LEFT JOIN
    ciam.mfa_total_mfa_users m ON g.user_id = m.username
WHERE
    CAST(a.event_date AS DATE) >= DATE_ADD('year', -1, CURRENT_DATE)
    AND CAST(o.event_date AS DATE) >= DATE_ADD('year', -1, CURRENT_DATE)
GROUP BY
    a.event_date,
    a.host,
    o.application,
...
account_dim_sum_df = read_athena_tables(account_dim_sum,account_dim_sum_temp)
profile_dim_sum_df = read_athena_tables(profile_dim_sum,profile_dim_sum_temp)
transaction_adobe_fact_df = read_athena_tables(transaction_adobe_fact,transaction_adobe_fact_temp)
transaction_okta_user_agg_fact_df = read_athena_tables(transaction_okta_user_agg_fact,transaction_okta_user_agg_fact_temp)
transcation_okta_day_agg_df = read_athena_tables(transcation_okta_day_agg,transcation_okta_day_agg_temp)
...
account_dim_sum_df_write_df = write_csv_files_into_s3(account_dim_sum_df,account_dim_sum_output,account_dim_sum_partitionkeys)
profile_dim_sum_write_df = write_csv_files_into_s3(profile_dim_sum_df,profile_dim_sum_output,profile_dim_sum_partitionkeys)
transaction_adobe_fact_write_df = write_csv_files_into_s3(transaction_adobe_fact_df,transaction_adobe_fact_output,transaction_adobe_fact_partitionkeys)
transaction_okta_user_agg_fact_write_df = write_csv_files_into_s3(transaction_okta_user_agg_fact_df,transaction_okta_user_agg_fact_output,transaction_okta_user_agg_fact_partitionkeys)
transcation_okta_day_agg_write_df = write_csv_files_into_s3(transcation_okta_day_agg_df,transcation_okta_day_agg_output,transcation_okta_day_agg_partitionkeys)
