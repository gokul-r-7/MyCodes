...
raw_datasets=["e3d"]
csv_datasets=["mto"]
curated_datasets=["elecequilistrep","elecequiwbsrep","electraywbsrep","instequiwbseep","pipecompwbsrep","pipelistrep","strcbeamwbsrep","strcfdnswbsrep","strcpanewbsrep"]


glue_job_names = {
    "e3d_mto_sourcing": f"worley-datalake-sydney-{environment}-glue-job-smb-connector",
    "e3d_mto_csv_xlsx_parsing": f"worley-datalake-sydney-{environment}-glue-job-csvxlsx-data",
    "e3d_mto_raw_curated": f"worley-datalake-sydney-{environment}-glue-job-raw-to-curated-generic"
}

with DAG(
    dag_id=f"e3d_mto_{environment}_engineering_data_pipeline", #E3d_mto_Files_data_pipeline
    schedule="35 8 * * 1-5",
    default_args=default_args,
    tags=["engineering_e3d_mto"],
    ...
    @task_group(group_id="e3d_mto_source_extraction")
    def e3d_mto_source_extraction():
        for src_file in raw_datasets:
            e3d_mto_source_extraction = GlueJobOperator(
                task_id=f"e3d_mto_source_extraction_{src_file}",
                job_name=glue_job_names["e3d_mto_sourcing"],
                region_name=region,
                verbose=True,
    ...
    @task_group(group_id="e3d_mto_csv_xlsx_task")
    def e3d_mto_csv_xlsx_task(): 
        for src in csv_datasets:
            e3d_mto_csv_xlsx_task = GlueJobOperator(
                task_id=f"e3d_mto_csv_xlsx_parsing_{src}",
                job_name=glue_job_names["e3d_mto_csv_xlsx_parsing"],
                region_name=region,
                verbose=True,
                wait_for_completion=True,
                script_args={
    ...
    raw_crawler = GlueCrawlerOperator(
            task_id=f"e3d_mto_csv_xlsx_raw_crawler",
            config={
    ...
    def raw_to_curated():
        for table in curated_datasets:
            raw_curated = GlueJobOperator(
                ...
                job_name=glue_job_names["e3d_mto_raw_curated"],
                region_name=region,
                verbose=True,
                wait_for_completion=True,
                script_args={
    ...
    batch_run_start_time_str >> e3d_mto_source_extraction() >> e3d_mto_csv_xlsx_task() >> raw_crawler >> raw_crawler_sensor >>  raw_to_curated()
