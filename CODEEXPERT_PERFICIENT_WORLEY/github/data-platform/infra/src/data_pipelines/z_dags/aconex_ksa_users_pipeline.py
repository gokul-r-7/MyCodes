...
# Datasets for Downstream Document Control for ksa instance
...
endpoint_host ="ksa1.aconex.com"
instance_name= "ksa_instance"
...
# Create DAG scheduled for every friday KSA 12 AM
with DAG(
    ...
    @task_group(group_id="run_ksa_aconex_users_glue_jobs")
    def run_ksa_users_glue_jobs(ProjectId):
        """Task Group to run all Glue jobs in parallel for a given project ID"""
        glue_job_tasks = []

        # User Project Task
        user_project_task = GlueJobOperator(
                task_id=f"aconex_ksa_user_project",
        ...
        user_projectrole_task = GlueJobOperator(
                task_id=f"aconex_ksa_user_projectrole",
    ...
    UserDirectory_job = GlueJobOperator(
        task_id="aconex_ksa_userDirectory",
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"aconex_ksa_users_raw_crawler",
        config={
    ...
    aconex_project_list_task >> run_ksa_users_glue_jobs.expand(ProjectId=aconex_project_list_task) >> raw_crawler >> raw_crawler_sensor
