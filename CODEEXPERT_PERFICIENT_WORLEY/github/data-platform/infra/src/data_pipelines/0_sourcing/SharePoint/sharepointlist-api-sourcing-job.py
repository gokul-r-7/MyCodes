...
logger.info(f" SharePoint Export Config :{export_config}")

############## Load SharePoint Export API Config ################
...
#############  Generate Dynamic Config for SharePoint Export API  ################
...
idx_typ_AI = export_config['job_parameter']['Assumption Index']
print("idx_typ_AI--->",idx_typ_AI)
...
try:
   for idx in titles:
    ...
    export_config['api_parameter']['endpoint']=f"{api_endpoint}"
    ...
    try:
        if response:
            if s3_client.upload_to_s3(response_bytes,object_name,kms_key_id,is_gzip=False):
                logger.info(f"Uploaded SharePoint info to {bucket_name}/{object_name}")
                success = True
            else:
                logger.error("SharePoint Export API was successful but the gzip content could not be uploaded to S3")
                success = False
        else:
            logger.error("Failed to fetch SharePoint API payload")
            success = False  
    ...
    if success:
      # Load whole XML file into a DynamicFrame
        ...
        root_df_rf_relationalize = Relationalize.apply(
            ...
            name=f"SharePoint",
            transformation_ctx=f"relationalize__SharePoint"
          )
        ...
        if 'SharePoint' in tables:
            main_df = tables['SharePoint']
        else:
            main_df = None
        ...
        logger.info(f"write_glue_df_to_s3_with_specific_file_name completed for the main data SharePoint {idx}")
        ...
        logger.info(f"write_glue_df_to_s3_with_specific_file_name completed for the sample data SharePoint {idx}")
