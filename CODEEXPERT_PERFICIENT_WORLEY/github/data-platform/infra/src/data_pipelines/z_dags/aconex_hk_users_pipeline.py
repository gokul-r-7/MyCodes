...
endpoint_host ="hk1.aconex.com"
instance_name= "hk1_instance"
...
# Create DAG
with DAG(
    dag_id=f"aconex_{environment}_{domain_name}_{instance_name}_users_data_pipeline", #aconex_hk_instance_data_pipeline
    schedule="00 16 * * 6",
    ...
    @task_group(group_id="run_hk_aconex_users_glue_jobs")
    def run_hk_users_glue_jobs(ProjectId):
        """Task Group to run all Glue jobs in parallel for a given project ID"""
        glue_job_tasks = []
        # User Project Task
        user_project_task = GlueJobOperator(
                task_id=f"aconex_hk_user_project",
        ...
        user_projectrole_task = GlueJobOperator(
                task_id=f"aconex_hk_user_projectrole",
    ...
    UserDirectory_job = GlueJobOperator(
        task_id="aconex_hk_userDirectory",
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"aconex_hk_users_raw_crawler",
        config={
    ...
    aconex_project_list_task >> run_hk_users_glue_jobs.expand(ProjectId=aconex_project_list_task) >> raw_crawler >> raw_crawler_sensor
