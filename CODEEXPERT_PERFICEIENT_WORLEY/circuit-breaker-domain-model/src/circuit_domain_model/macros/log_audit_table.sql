{% macro log_audit_table(results) %} 
    ...
    {%- if execute -%}
        ...
        {%- set parsed_results = parse_dbt_results(results) or [] -%}
        {{ print("Parsed DBT Results: " ~ parsed_results) }}
            ...
            {% set insert_dbt_results_query -%}
                insert into {{ ref('audit_table') }}
                    ...
                    {%- for parsed_result_dict in parsed_results -%}
                        (
                           '{{ parsed_result_dict.get('model_load_id', '') }}',
                            ...
                            {{ parsed_result_dict.get('execution_time', 0) }},
                            {{ parsed_result_dict.get('rows_affected', 0) }},
                            {{ run_date }}
