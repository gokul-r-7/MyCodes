...
dag_spid_config = Variable.get("spid_env", deserialize_json=True)
dag_config = Variable.get("env", deserialize_json=True)
spid_environment = dag_spid_config["environment"]
environment = dag_config["environment"]
region = "ap-southeast-2"
source_system_id = "spid"
domain_name = "engineering"

table_names = ["T_Equipment", "T_Drawing", "T_Instrument", "T_PlantItem", "T_InstrFailMode", "T_Representation", "T_PipingComp", "T_PipeRun", "T_Vessel", "T_Mechanical", "T_Exchanger", "T_PlantGroup", "codelists", "Enumerations"]
...
glue_job_names = {
    ...
    "spid_raw_curated": f"worley-datalake-sydney-{environment}-glue-job-raw-to-curated-generic",
    "schema_change_detection": f"worley-datalake-sydney-{environment}-glue-job-schema-change-detection-generic"
}

payload = json.dumps({
          "dag_name": f"spid_{environment}_data_pipeline"
          })
...
# Create DAG
with DAG(
    dag_id=f"spid_{environment}_data_pipeline",
    schedule="30 7 * * 1-5",
    tags=["spid"],
    ...
    raw_crawler = GlueCrawlerOperator(
        task_id=f"spid_raw_crawler",
        config={
            "Name": f"worley-datalake-sydney-{environment}-glue-crawler-engineering-spid-raw",
        },
        poll_interval=5,
    ...
    def raw_to_curated():
        for table in table_names:
            raw_curated = GlueJobOperator(
                task_id=f"cur_{source_system_id}_{table}",
                job_name=glue_job_names["spid_raw_curated"],
                region_name=region,
                verbose=True,
                wait_for_completion=True,
                script_args={
                    "--source_system_id": f"database_{source_system_id}_curated",
                    "--metadata_type": f"curated#spid#{table}#job#iceberg",
                    "--start_date": execution_date,
                    "--end_date": execution_date,
                },
            )
